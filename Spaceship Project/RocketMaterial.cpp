...
{
	m_vsFileName = "ShaderRocket";
	m_psFileName = "ShaderRocket";
}
...
void RocketMaterial::Deinit()
{
	Material::Deinit();
}

void RocketMaterial::SetMaterial(const Object * pObject)
{
	...
	// Параметры SpotLight
	const double spot_angle_narrow = cos(PI / 50);
	const double spot_angle_wide = cos(PI / 7);
	const double spot_intensity = 2.0f;

	// Параметры Specular
	const double blick_intensity = 300;
	...
	SetMaterialBegin();
	{
		...
		SetPixelShaderVector4("spot_angle_narrow", Vector4(0, 0, 0, spot_angle_narrow));
		SetPixelShaderVector4("spot_angle_wide", Vector4(0, 0, 0, spot_angle_wide));
		SetPixelShaderVector4("blick_intensity", Vector4(0, 0, 0, blick_intensity));
		SetPixelShaderVector4("spot_intensity", Vector4(0, 0, 0, spot_intensity));
		SetPixelShaderVector4("CamPos", { SceneUtils::GetEyePosition(), 0 });
